...
IW_DECL(IppiSizeL) iwiColorGetPlaneSize(
    IwiColorFmt color,    // Color format
    IppiSizeL   origSize, // Full plane image size
    int         planeNum  // Number of plane for planar format, from 0. Ignored for interleaved formats
);
...
IW_DECL(IppStatus) iwiColorConvert(
    ...
    IwiRoi                *pRoi         // [in,out] Pointer to IwiRoi structure for tiling. If NULL - the whole image will be processed in accordance to size
);
