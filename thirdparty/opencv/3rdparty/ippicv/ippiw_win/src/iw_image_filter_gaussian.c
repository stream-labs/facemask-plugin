...
typedef IppStatus (__STDCALL *IppiFilterGaussian_ptr)(const void* pSrc, IppSizeL srcStep, void* pDst, IppSizeL dstStep,
...
static void __STDCALL tlsDescturctor(void* pParams)
{
    ...
    if(pSpec->extended && pSrcImage->m_channels != 1)
    {
        ...
        for(roi.y = 0; roi.y < size.height; roi.y += roi.height)
        {
            for(roi.x = 0; roi.x < size.width; roi.x += roi.width)
            {
                ...
                status = iwiCopySplit(pSrcImage, pTls->pInter_split, NULL, &pTls->pipe[0]);
                if(status < 0)
                    return status;
                ...
                status = iwiCopyMerge((const IwiImage* const*)pTls->pInter_proc, pDstImage, NULL, &pTls->pipe[2]);
                if(status < 0)
                    return status;
            }
        }
