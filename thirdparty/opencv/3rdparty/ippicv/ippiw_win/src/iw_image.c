...
IW_DECL(IppStatus) iwiImage_Alloc(IwiImage *pImage, IwiSize size, IppDataType dataType, int channels, IwiBorderSize const *pInMemBorder)
{
    ...
    if(allocSize.height > 1)
    {
        if(step*allocSize.height < 64)
            step = step;
        else if(step < 16)
            step = 16;
        else if(step < 32)
...
IW_DECL(IppStatus) iwiTilePipeline_SetRoi(IwiTile *pTile, IwiRoi tileRoi)
{
    ...
    while(pTile)
    {
        ...
        else
        {
            ...
            if(!pTile->m_pChild) // Correction is required only for the first tile int the pipe since all other tiles use iwiCopyMakeBorder function
            {
