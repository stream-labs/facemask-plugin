...
typedef IppStatus (__STDCALL *IppiFilterMorphology_ptr)(const void* pSrc, IppSizeL srcStep, void* pDst, IppSizeL dstStep,
                IppiSizeL roiSize, IppiBorderType borderType, const void *borderValue, const void* pMorthSpec, Ipp8u* pBuffer);

typedef IppStatus (__STDCALL *IppiFilterMorphology_GetBufferSize_ptr)(IppiSizeL roiSize, IppiSizeL maskSize, IppDataType depth, int numChannels, IppSizeL* bufferSize);
...

    {
        ...
        for(;;)
        {
            ...
            default:
                ...
                if(status < 0)
                    break;
                status = ippiMorphSetMode_L(0, (IppiMorphAdvStateL*)pSpec->pIppSpec); break;
            }
            break;
        }
